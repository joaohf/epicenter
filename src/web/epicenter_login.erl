-module(epicenter_login).
-compile(export_all).
-include_lib("nitrogen_core/include/wf.hrl").
-include("epicenter.hrl").

main() ->
    #template{file = code:priv_dir(epicenter) ++ "/templates/login.html"}.
...
event(login) ->
    ...
    case epicenter_user_server:authenticate(Username, Password, RememberMe, OldRememberMeSeries) of
        {error, bad_auth} ->
        ...
        {ok, Roles, RememberMeToken} ->
            ...
            wf:redirect_from_login("/epicenter/main")
    end.
