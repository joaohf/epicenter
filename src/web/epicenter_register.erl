...
header() ->
    epicenter_common:header(register).
...
event(register_user) ->
    ...
    case epicenter_user_server:register_user(Username, Password, Email) of
        {ok, _User} ->
...
user_not_exists(_Tag, Username) ->
    not epicenter_user_server:user_exists(Username).

email_not_registered(_Tag, Email) ->
    not epicenter_user_server:email_registered(Email).
